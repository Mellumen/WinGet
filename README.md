...
* **Component:** [Winget-AutoUpdate (WAU)](https://github.com/Romanitho/Winget-AutoUpdate) by Romanitho. The `.intunewin` package is built using `winget-install.ps1` and the `functions` folder (sourced from `Romanitho/Winget-AutoUpdate/Sources`).
...
This process uses an Intune Remediations script to copy WAU logs to `...\IntuneManagementExtension\Logs\`.
This is to ensure Intune "Collect diagnostics" also collects the latest WAU logs for monitoring and troubleshooting.

* **Detection Script:** `WAULogs_detection.ps1`
    * Monitors `install.log` and `updates.log` from Winget-AutoUpdate.
...
* **Remediation Script:** `WAULogs_remediation.ps1`
    * Copies the latest WAU log files to the IME logs directory.
    * Renames the files with a `WAU-` prefix for easy identification (e.g., `WAU-install.log`).
